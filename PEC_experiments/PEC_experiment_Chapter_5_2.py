"""
========================================================================
PEC Experiments (section 5.2) — Parameter Estimation and Calibration
------------------------------------------------------------------------
Context:
    This executable reproduces the computational workflow reported in
    Chapter 5, Section 5.2 of the thesis, and is included as supplementary
...
output_pdf = "PEC_Chapter5_2_Report.pdf"
output_json = "PEC_5_section_5_2.jsonl"

PEC.set_model_mode("1param") # For experiments calibrating Lambda
print(f"Executing: Chapter 5 - Section 2 experiment")
...
def misfitH(param):
    """
    Light version of the misfit function associated with lambda, used for heuristics. 
...
lambda_values = np.linspace(Boundsinf[0], Boundsup[0], 3*PEC.grid_size_b)
...
with ProcessPoolExecutor(max_workers=get_n_workers(), initializer=PEC.set_model_mode,
    initargs=("1param",)) as executor:
    futures = [executor.submit(list_misfitH, p) for p in lambda_values]
...
MisfitHeuristic = [lambda_values.tolist(),f_list.tolist()]

print("Reached part 2: Heuristic finished.")

min_index = np.argmin(f_list)
InitialGuess = lambda_values[min_index]
